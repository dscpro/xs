...
$('btn').getElementsByTagName('span')[0].className = 'current';
...
if (xpos > final_x) {
var dist = Math.ceil((xpos - final_x)/10);//Math.ceil 向上取整
xpos = xpos - dist;
}
if (ypos < final_y) {
...
var btnList = $('btn').getElementsByTagName('span');
...
//循环图片设置叠加层    覆盖
function iBanner() {
if(!$('ibanner')||!$('ibanner_pic')||!$('btn')) return;
...
function autoBanner() {
if(!$('ibanner')||!$('ibanner_pic')||!$('btn')||autoKey) return;
var btnList = $('btn').getElementsByTagName('span');
var picList = $('ibanner_pic').getElementsByTagName('a');
if (picList.length==1) return;
for(var i=0; i<btnList.length; i++) {
